    ...
    <VisitPostWrapper>
      <Navbar />
        <VisitPostContentbreadcrumbs>
             <VisitPostContentHeaderItem>{post.category} <ArrowBackIosIcon /></VisitPostContentHeaderItem>
             <VisitPostContentHeaderItem>{post.subCategory}<ArrowBackIosIcon /></VisitPostContentHeaderItem>
             <VisitPostContentHeaderItem>{post.title}<ArrowBackIosIcon /></VisitPostContentHeaderItem>
        </VisitPostContentbreadcrumbs>
         < VisitPostContentWrapper>
          <VisitPostContentRightSection>
            <PostTitle >{post.title}</PostTitle>
            <PostDateInformation onClick={() => setOpenDetail(prev=> !prev)}>{dayjs(post.date).fromNow()} در  {post.city} <div style={{ transform: openDetail ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.3s ease' }}> <KeyboardArrowDownIcon /></div></PostDateInformation>
          
               <PostDateInfomarionSumMenu $opened ={openDetail}>
              <div> انتشار آگهی :{dayjs(post.date).format('DD MMMM YYYY')}</div>
              <div>آخرین بروزرسانی : {dayjs(post.date).format('DD MMMM YYYY')}</div>
            </PostDateInfomarionSumMenu>
            <PostWarning><div><WarningAmberRoundedIcon /> زنگ های خطر قبل از معامله  </div><ArrowBackIosIcon/></PostWarning>
            <PostInformation>
                <PostInformationHeader >
                    <UserInformationBtn >اطلاعات  تماس</UserInformationBtn>
                    <ChatUserBtn >چت</ChatUserBtn>
                    <PostInformationIcon><BookmarkBorderRoundedIcon /></PostInformationIcon>
                    <PostInformationIcon><ShareRoundedIcon /></PostInformationIcon>
                </PostInformationHeader>
  ...
  <PostTbody>
    <PostTr>
      ...
      <PostTd>سفید</PostTd>
        ...
        <VisitPostFooter></VisitPostFooter>
      </VisitPostContentWrapper>
    </VisitPostWrapper>
  );
}
